...
def primeSieve(n_max: int, sieve: List[bool], primes: List[int]) -> None:
    """
    Updates input sieve and primes in place
    """
    if n_max < len(sieve): return
    n_orig = len(sieve) - 1
    if n_max < 2:
        sieve.extend([False] * (n_max - n_orig))
        return
    sieve.extend([True] * (n_max - n_orig))
    sieve[0] = False
    sieve[1] = False
    if n_orig < 2: primes.append(2)
    if n_max == 2: return
    for i in range(max(4, n_orig + 2 - (n_orig & 1)), n_max + 1, 2):
        sieve[i] = False
    p = 3
    for (iter_end, rng_start_func) in ((n_orig, lambda: n_orig + p - (n_orig % p)), (n_max, lambda: sq)):
        for p in range(p, n_orig + 1, 2):
            if not sieve[p]: continue
            sq = p ** 2
            if sq > iter_end: break
            for i in range(rng_start_func(), n_max + 1, p):
                sieve[i] = False
        else:
            for p in range(n_orig + 1 + (n_orig & 1), n_max + 1, 2):
                if not sieve[p]: continue
                primes.append(p)
                sq = p ** 2
                if sq > n_max: break
                for i in range(sq, n_max + 1, p):
                    sieve[i] = False
            break
    for p in range(max(p + 2, n_orig + 1 + (n_orig & 1)), n_max + 1, 2):
        if sieve[p]:
            primes.append(p)
    return

def primeFactors(n: int, primes: Optional[List[int]]=None) -> Set[int]:
    res = set()
    if primes is None:
        # If the list of primes up to at least n is not provided, uses
        # 2 and the odd integers from 3 to n (which contains all the
        # primes up to n) rather than finding these primes, which is not
        # worth it for finding the prime factors of a single number
        primes = (p + (p == 1) for p in range(1, n + 1, 2))
    for p in primes:
...
def primeFactorisation(n: int, primes: Optional[List[int]]=None) -> Dict[int, int]:
    res = {}
    if primes is None:
        # If the list of primes up to at least n is not provided, uses
        # 2 and the odd integers from 3 to n (which contains all the
        # primes up to n) rather than finding these primes, which is not
        # worth it for a single prime factorisation
        primes = (p + (p == 1) for p in range(1, n + 1, 2))
    for p in primes:
        if n == 1 or p ** 2 > n: break
        n2, r = divmod(n, p)
...
def primeSPFsieve(n_max: int, sieve: List[int]) -> None:
    """
    Finds the smallest prime factor for each number. Updates input sieve in place
    """
    if n_max < len(sieve): return
    n_orig = len(sieve) - 1
    if n_max < 2:
        sieve.extend([-1] * (n_max - n_orig))
        return
    sieve.extend(list(range(n_orig + 1, n_max + 1)))
    sieve[0] = -1
    sieve[1] = -1
    if n_max <= 3: return
    for i in range(max(4, n_orig + 2 - (n_orig & 1)), n_max + 1, 2):
    ...
    for p in range(3, n_max + 1, 2):
        ...
        sq = p ** 2
        for i in range(max(sq, n_orig + p - (n_orig % p)), n_max + 1, p):
            sieve[i] = min(sieve[i], p)
    return

def primeFactorsSPFsieve(n: int, sieve: List[int]) -> Set[int]:
    primeSPFsieve(n, sieve)
    res = set()
    while sieve[n] != n:
        p = sieve[n]
        res.add(p)
        while n != p and sieve[n] == p:
            n //= p
        if n == p: return res
    res.add(n)
    return res

def primeFactorisationSPFsieve(n: int, sieve: Dict[int, int]) -> Dict[int, int]:
    primeSPFsieve(n, sieve)
    res = {}
    while sieve[n] != n:
        p = sieve[n]
...
def primeSPFsieve2(n_max: int, sieve: List[Optional[Tuple[int]]]) -> None:
    """
    Finds the smallest prime factor for each number and the number of times
    it divides the number. Updates input sieve in place
    """
    if n_max < len(sieve): return
    n_orig = len(sieve) - 1
    if n_max < 2:
        sieve.extend((-1, 0, 1) * (n_max - n_orig))
        return
    sieve.extend([(x, 1, 1) for x in range(n_orig + 1, n_max + 1)])
    sieve[0] = (-1, 0, 1)
    sieve[1] = (-1, 0, 1)
...
def primeFactorsSPFsieve2(n: int, sieve: List[int]) -> Set[int]:
    primeSPFsieve2(n, sieve)
    res = set()
    while sieve[n] != n:
        res.add(sieve[n][0])
        n = sieve[n][2]
...
def primeFactorisationSPFsieve2(n: int, sieve: Dict[int, int]) -> Dict[int, int]:
    primeSPFsieve2(n, sieve)
    res = {}
    while sieve[n] != n:
        res[sieve[n][0]] = sieve[n][1]
...
def findValidSplit(self, nums: List[int]) -> int:
    """
    Solution to Leetcode #2584 using smallest prime factor (SPF) sieve
    to find factors
    """
    n = len(nums)
    if n < 2: return -1
    p_intvls = {}
    for i in range(n):
        facts = primeFactorsSPFsieve(nums[i], sieve)
        for p in facts:
            if p not in p_intvls:
                p_intvls[p] = [i, i]
            else: p_intvls[p][1] = i
    for p in list(p_intvls.keys()):
        if p_intvls[p][0] == p_intvls[p][1]: p_intvls.pop(p)
    intervals = sorted(p_intvls.values())
    if not intervals or intervals[0][0]: return 0
    res = intervals[0][1]
    for i in range(1, len(intervals)):
        if intervals[i][0] >= res: return res
        res = max(res, intervals[i][1])
